---
aliases:
  - ../data-sources/azure-prometheus/configure/
  - ../data-sources/azure-monitor-prometheus/configure/
...
labels:
  products:
    ...
    - oss
menuTitle: Configure
title: Configure the Azure Monitor Managed Service for Prometheus data source
weight: 100
...
1. Type `Azure Monitor Managed Service for Prometheus` in the search bar.
1. Select **Azure Monitor Managed Service for Prometheus**.
...
The data source supports [Private data source connect (PDC)](https://grafana.com/docs/grafana-cloud/connect-externally-hosted/private-data-source-connect/), which lets Grafana Cloud query an Azure Monitor workspace that isn't exposed to the public internet.
...
1. In **Advanced HTTP settings**, add a custom header named `x-ms-azure-scoping` whose value is the Azure resource ID, resource group ID, or subscription ID.
1. Authenticate with an identity that has at least the **Monitoring Reader** role on that scope.
...
To verify the connection, click **Save & test**. When the configuration is valid, Grafana displays a success message such as **Successfully queried the Prometheus API.** If the test fails, refer to [Troubleshooting](https://grafana.com/docs/plugins/grafana-azureprometheus-datasource/latest/troubleshooting/).
