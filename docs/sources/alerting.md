...
To create a Grafana-managed alert rule, refer to [Configure Grafana-managed alert rules](https://grafana.com/docs/grafana/<GRAFANA_VERSION>/alerting/alerting-rules/create-grafana-managed-rule/). Set the rule's query to a PromQL expression that returns a numeric value, then add a threshold condition.

{{< admonition type="note" >}}
Grafana-managed alert rules require credentials that work without a signed-in user. If the data source uses **Current User** authentication, configure [fallback service credentials](https://grafana.com/docs/plugins/grafana-azureprometheus-datasource/latest/configure/#current-user) so rule evaluation has an identity.
{{< /admonition >}}
